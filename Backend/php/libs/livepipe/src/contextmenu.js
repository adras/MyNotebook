...
//  f(!Control.ContextMenu.enabled || (Prototype.Browser.Opera && !event.ctrlKey)) {
// Crazy logic, the user has to press CTRL in opera even for left clicks. CTRL + leftclicking unfortunatelly opens a file save dialog in Opera 10.10.
// Added another check so the user has to press CTRL only when the context menu is opened by right clicking	
            if(!Control.ContextMenu.enabled || (Prototype.Browser.Opera && !event.ctrlKey && !this.options.leftClick)) {
                return; }
            this.open(event);
